...
instance.interceptors.request.use(function (config) {
  ...
  if (store.state.user.token) {
    config.headers.Authorization = `Bearer ${store.state.user.token}` // 将token 统一注入到header中
  }
  // 不用if else的用法
...
instance.interceptors.response.use(function (response) {
  ...
  // 如果请求失败或者失效或者其他错误 会进入到 响应拦截器的错区区域
  // 只要是 401  就是token失效(应该用导航守卫将没有token的拦截在外面)
  /*****
     * error
     *  config 是出现这次错误的配置信息
     *  request 请求对象
  ...
  if (error.response && error.response.status === 401) {
    ...
    // 401状态码  默认 token 失效  需要处理toke失效问题
    ...
    if (store.state.user.refresh_token) {
      ...
      // 如果没有refresh_token  直接宣布over 直接跳的登录页面
      // router.push('/login') //直接跳转登录 需要做更复杂的场景
